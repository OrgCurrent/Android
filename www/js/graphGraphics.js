    ...
    animate: function(data, margin) {
      ...
      for (var i = 0; i < data.length; i++) {
        var numScores = data[i].scores.length;
          ...
          } else console.log(numScores);
      }
      // recentScores now contains an array of most recent score for each user.
      ...
      var dataPoints = recentScores.length;
      ...
      var placePoints = function(data) {
        ...
        others.enter()
          ...
          .attr({
            ...
            .transition().delay(function(d, i) { return 50 * i }).duration(200)
          .attr('r', 5)
          .each('end', function() {
            progressBarUpdate();
            ripples([this.cx.animVal.value, this.cy.animVal.value]);
      ...
      var lin = ss.linear_regression().data(recentScores.map(function(d) {
        return [translateX(d[0]), translateY(d[1])];
      })).line();

      // Create a line based on the beginning and endpoints of the range
      var linData =  recentScores.map(function(d) {
        return {
          x: translateX(d[0]),
          y: lin(translateX(d[0]))
        };
      });
